...
export class StockService {
  ...
  constructor(readStock: ReadStockRepository, writeStock: WriteStockRepository) {
  ...
  async deleteStock(stockID: number, userID: number) {
    ...
    rootMain.info('deleteStock result affectedRows:', result.affectedRows);
    if (result.affectedRows === 0) {
  ...
  async getLowStockItems(UserID: number) {
    ...
    rootMain.info('Items à faible stock récupérés:', lowStockItems);
