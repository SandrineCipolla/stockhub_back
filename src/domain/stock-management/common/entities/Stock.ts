...
export class Stock {
  ...
  static create(params: {
    ...
    const stockLabel =
      params.label instanceof StockLabel ? params.label : new StockLabel(params.label);

    const stockDescription =
      params.description instanceof StockDescription
        ? params.description
    ...
    return new Stock(params.id ?? 0, stockLabel, stockDescription, params.category.trim(), []);
  }
  ...
  updateInfo(newLabel: StockLabel | string, newDescription: StockDescription | string): void {
    this.label = newLabel;
  ...
  updateItemQuantity(itemId: number, newQuantity: number | Quantity): void {
    const quantity = newQuantity instanceof Quantity ? newQuantity.value : newQuantity;
