...
| accès API              | `fetch` natif, pas de client HTTP dédié                                  | appels REST vers `/api/v1` et `/api/v2`            | [ADR-012](https://github.com/SandrineCipolla/stockHub_V2_front/blob/main/docs/adr/ADR-012-fetch-natif-plutot-quaxios.md) | si le nombre de call sites dépasse ~15-20 fichiers, ou besoin d'intercepteur (refresh token) |
