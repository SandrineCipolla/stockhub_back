...
export function createAzureAuthHelper(): AzureAuthHelper {
    ...
    const config: AzureB2CConfig = {
        ...
        tenantName: process.env.AZURE_TENANT_ID!,
        b2cDomain: process.env.AZURE_B2C_DOMAIN!,
        policyName: process.env.AZURE_B2C_POLICY!,
        username: process.env.AZURE_TEST_USERNAME!,
        password: process.env.AZURE_TEST_PASSWORD!,
        // Use the scope defined in ROPC_Auth_app
        scopes: [`https://stockhubb2c.onmicrosoft.com/${process.env.AZURE_CLIENT_ID}/access_as_user`],
    };
