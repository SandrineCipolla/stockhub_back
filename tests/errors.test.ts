...
describe('sendError', () => {
    ...
    it('should return 500 for DatabaseError', () => {
        const err = new DatabaseError('Database error',ErrorMessages.DeleteStockItem, new Error('Original DB error'));
        sendError(res, err);
    ...
    it('should return 500 for unknown errors', () => {
        ...
        sendError(res, err);
        expect(res.status).toHaveBeenCalledWith(HTTP_CODE_INTERNAL_SERVER_ERROR);
        expect(res.json).toHaveBeenCalledWith({error: 'An unexpected error occurred. Please try again later.'});
    });
});
